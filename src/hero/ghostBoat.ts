...
export async function mountGhostBoat(ocean: Ocean, copy: HTMLElement): Promise<void> {
  ...
  ocean.onDispose = () => { layout.disconnect(); ocean.scene.remove(boat); material.dispose() }
  ...
  ocean.onFrame = (dt, t) => {
    ...
    const depth = portrait ? 5.7 : DEPTH
    ...
    const scale = portrait ? 1.2 : Math.min(3.0, 2.85 * ocean.camera.aspect / 1.6)
    ...
    boat.position.x = halfWidth * (portrait ? .16 : targetX)
    if (!portrait) {
      ...
      for (let pass = 0; pass < 2; pass++) {
        ...
        boat.position.x += (targetX - (left + right) / 2) * halfWidth
      }
    }
    ...
    boat.position.y = heave + (portrait ? .3 : .35)
