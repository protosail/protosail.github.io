...
export async function mountGhostBoat(ocean: Ocean, copy: HTMLElement): Promise<void> {
  ...
  let sceneWidth = 1, viewportWidth = 1
  ...
  const measure = () => {
    ...
    const viewportLeft = heroRect.left - sceneRect.left
    const viewportRight = viewportLeft + hero.clientWidth
    const textRight = Math.max(...Array.from(copy.children, child => child.getBoundingClientRect().right))
    const copyRight = textRight - sceneRect.left
  ...
  ocean.onFrame = (dt, t) => {
    ...
    const depth = mobile ? (portrait ? 12.5 : 9.5) : DEPTH
    // Portrait phones use a deeper, proportionally larger model so the complete
    // silhouette stays in the water while occupying the middle of the viewport.
    const scale = mobile
      ? (portrait ? 2.7 : 1.55)
    ...
    // Phones center the projected silhouette behind the copy; wider layouts keep
    // centering it in the open space to the right of the text.
    boat.position.x = mobile
      ? (portrait ? 0 : halfWidth * mobileLandscapeTargetX)
      : halfWidth * targetX
    if (!portrait || mobile) {
      const horizontalTarget = mobile ? (portrait ? 0 : mobileLandscapeTargetX) : targetX
